...
import "./ClubMarquee.scss";
import { useRef } from "react";
import { usePageData } from "@lib/hooks/usePageData";
import { YesOrNo } from "@lib/services/notice/notice.service.type";

const ClubMarquee: React.FC = () => {
  const {
    latestNews,
    pageLatestNews,
    pageNews,
  ...
  const marqNews = pageNews?.filter((n) => n.isMarquee === YesOrNo.YES);
  const marqNotices = pageNotices?.filter((n) => n.isMarquee === YesOrNo.YES);
  const marqEvents = pageEvents?.filter((n) => n.isMarquee === YesOrNo.YES);
  const marqAchievements = pageAchievements?.filter(
    (n) => n.isMarquee === YesOrNo.YES
  );
  ...
  const normalize = () => {
    return [
      ...
      ...marqNews?.map((item) => ({
        id: item.id,
        label: item.label,
        link: `/pages/news-details/${item.id}`,
      })),
      ...marqNotices?.map((item) => ({
        id: item.id,
        label: item.label,
        link: `/pages/notice-details/${item.id}`,
      })),
      ...marqEvents?.map((item) => ({
        id: item.id,
        label: item.title,
        link: `/pages/event-details/${item.id}`,
      })),
      ...marqAchievements?.map((item) => ({
        id: item.id,
        label: item.label,
        link: `/pages/achievement-details/${item.id}`,
      })),
    ];
  };
